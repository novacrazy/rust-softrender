...
pub struct PixelRef<'a, T: 'a>(pub ( in ::pixels) usize,
                               pub ( in ::pixels) &'a T) where T: PixelRead;

/// Mutable reference to a pixel
///
/// Provides a writable accessor for the pixel at the coordinates given at creation.
pub struct PixelMut<'a, T: 'a>(pub ( in ::pixels) usize,
                               pub ( in ::pixels) &'a mut T) where T: PixelWrite;

impl<'a, T: 'a> PixelRef<'a, T> where T: PixelRead {
    #[inline(always)]
    pub ( in ::pixels ) fn new(index: usize, framebuffer: &'a T) -> PixelRef<'a, T> {
...
impl<'a, T: 'a> PixelMut<'a, T> where T: PixelWrite {
    #[inline(always)]
    pub ( in ::pixels ) fn new(index: usize, framebuffer: &'a mut T) -> PixelMut<'a, T> {
