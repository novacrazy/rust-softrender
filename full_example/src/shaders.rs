...
use ::color::{Color, SRGB_GAMMA, decode_gamma, encode_gamma};
use ::mesh::VertexData;
use ::uniforms::{Light, GlobalUniforms, Uniforms};
...
pub fn fragment_shader(vertex: &ScreenVertex<Uniforms>, global_uniforms: &GlobalUniforms) -> Fragment<Color> {
    ...
    let material_color = decode_gamma(Color { r: 0.2, g: 0.2, b: 0.8, a: 1.0 }, SRGB_GAMMA);
    ...
    for light in lights {
        ...
        color = Color {
            r: color.r + light.intensity * light.color.r * (specular + diffuse * material_color.r),
            g: color.g + light.intensity * light.color.g * (specular + diffuse * material_color.g),
            b: color.b + light.intensity * light.color.b * (specular + diffuse * material_color.b),
            a: color.a,
        };
    }

    Fragment::Color(encode_gamma(color, SRGB_GAMMA))
}